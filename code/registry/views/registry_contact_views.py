...
@login_required()
@require_http_methods(["POST"])
def registry_contact_edit(request: HttpRequest, registry_id: int, contact_id: int):
    registry_contact = get_object_or_404(RegistryContact, pk=contact_id)
    registry_contact_form = RegistryContactForm(request.POST, instance=registry_contact)
    if registry_contact_form.is_valid():
        registry_contact_form.save()
...
@login_required()
@require_http_methods(["POST"])
def registry_contact_delete(request: HttpRequest, registry_id: int, contact_id: int):
    registry_contact = get_object_or_404(RegistryContact, pk=contact_id)
    try:
