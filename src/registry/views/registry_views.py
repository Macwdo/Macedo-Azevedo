...
def registry_create(request: HttpRequest):
    if request.method == 'GET':
        ...
        registry_form_fields_id = [
            field.id_for_label for field in registry_form
        ]
        registry_cpf_form_fields_id = [
            field.id_for_label for field in registry_cpf_form
        ]
        registry_cnpj_form_fields_id = [
            field.id_for_label for field in registry_cnpj_form
        ]
            ...
            with transaction.atomic():
                if registry_form.is_valid():
                    ...
                    if registry_cpf_form.is_valid():
                        ...
                        if cpf:
                            registry.registry_cpf = RegistryCpf.objects.create(
                                **registry_cpf_form.cleaned_data
                            )

                    if registry_cnpj_form.is_valid():
                        cnpj = registry_cnpj_form.cleaned_data.get(
                            'cnpj', None
                        )
                        registry_cnpj_form.cleaned_data['registry'] = registry
                        if cnpj:
                            registry.registry_cnpj = (
                                RegistryCnpj.objects.create(
                                    **registry_cnpj_form.cleaned_data
                                )
                            )
                ...
                return redirect(
                    reverse(
                        ...
                        kwargs={'registry_id': registry.pk},
                    )
                )
        except:
            messages.error(
                request, 'Não foi possível efetuar a criação do registro'
            )
...
def registry_list(request: HttpRequest):
    ...
    context = {'registries': registries, 'page': page}
...
def registry_detail(request: HttpRequest, registry_id: int):
    ...
    lawsuits_envolved = Processos.objects.filter(
        Q(cliente=registry) | Q(parte_adversa=registry)
    )
...
def registry_edit(request: HttpRequest, registry_id: int):
    registry = get_object_or_404(Registry, pk=registry_id)
    registry_form = RegistryForm(
        request.POST, request.FILES, instance=registry
    )
    ...
    if registry_cpf_form.is_valid():
        ...
        if cpf:
            ...
            if registry_cpf.exists():
                ...
                registry_cpf_form.cleaned_data['registry'] = registry
                new_registry_cpf = RegistryCpf.objects.create(
                    **registry_cpf_form.cleaned_data
                )
    ...
    if registry_cnpj_form.is_valid():
        ...
        if cnpj:
            ...
            if registry_cnpj.exists():
                ...
                registry_cnpj_form.cleaned_data['registry'] = registry
                new_registry_cnpj = RegistryCnpj.objects.create(
                    **registry_cnpj_form.cleaned_data
                )
...
def registry_delete(request: HttpRequest, registry_id: int):
    ...
    try:
        registry.delete()
        messages.success(
            request, f'Registro {registry.name} foi excluído com sucesso'
        )
    except:
        messages.error(
            request, f'Não foi possível apagar o registro do {registry}'
        )
...
def registry_send_email(request: HttpRequest, registry_id):
    ...
    try:
        send_email_with_template(
            receivers=[receiver], context=context, files=files
        )
        ...
        messages.error(
            request,
            'Houve um erro com a senha do email raiz, favor contatar administrador.',
        )
        ...
        messages.error(
            request,
            'Houve um erro ao enviar o email, favor contatar administrador.',
        )
