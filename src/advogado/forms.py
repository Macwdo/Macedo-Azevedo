...
class LawyerForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        for k, v in self.fields.items():
            if self.fields[k].required is False:
                self.fields[k].label += ' *'
    ...
    class Meta:
        ...
        widgets = {
            ...
            'cpf': forms.TextInput(
                attrs={'class': 'form-control cpf', 'name': 'cpf'}
            ),
    ...
    def clean_user(self):
        ...
        if data.username == 'admin':
            raise ValidationError(
                'Não é possível o Advogado pertencer ao usuário admin'
            )
        try:
            lawyer = Advogado.objects.exists(user=data)
            if lawyer:
                raise ValidationError(
                    'Já existe um advogado aliado a esse usuário'
                )
        except:
            return data
