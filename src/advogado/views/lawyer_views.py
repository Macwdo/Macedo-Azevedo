...
def lawyer_create(request: HttpRequest):
    if request.method == 'GET':
        ...
        if form.is_valid():
            lawyer = form.save()
            messages.success(
                request, f'Advogado {lawyer.name} Registrado com sucesso.'
            )
...
def lawyer_list(request: HttpRequest):
    ...
    context = {'lawyers': lawyers, 'page': page}
...
def lawyer_detail(request: HttpRequest, lawyer_id):
    ...
    context = {'lawyer': lawyer, 'lawyer_form': lawyer_form}
...
def lawyer_edit(request: HttpRequest, lawyer_id: int):
    ...
    if lawyer_form.is_valid():
        ...
        messages.success(
            request, f'Advogado {lawyer.name} editado com sucesso'
        )
    else:
    ...
    return redirect(
        reverse('lawyer:lawyer_detail', kwargs={'lawyer_id': lawyer_id})
    )
...
def lawyer_delete(request: HttpRequest, lawyer_id: int):
    ...
    try:
        lawyer.delete()
        messages.success(
            request, f'Advogado {lawyer.name} foi excluído com sucesso'
        )
    except:
        messages.error(
            request, f'Não foi possível apagar o registro do { lawyer.name }'
        )
    return redirect(reverse('lawyer:lawyer_list'))
