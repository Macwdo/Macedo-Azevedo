...
class Processos(models.Model):
    posicao_choice = [('autor', 'Autor'), ('reu', 'Réu')]
    ...
    parte_adversa = models.ForeignKey(
        ...
        null=True,
        blank=False,
        related_name='lawsuit_adverse_part',
    )
    ...
    colaborador = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='lawsuit_colaborator',
    )
    ...
    indicado_por = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name='lawsuit_indicated_by',
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    def honorarios_registrados(self):
        return ProcessosHonorarios.objects.filter(
    ...
    def profit(self):
        ...
        for honorario in honorarios:
            total += (
                honorario.valor if honorario.ganho else (honorario.valor * -1)
            )
    ...
    def anexos_registrados(self):
        return ProcessosAnexos.objects.filter(
            processo=Processos.objects.get(pk=self.pk)
        ).order_by('-id')
...
class ProcessosMovimento(models.Model):
    ...
    def date(self):
        return datetime(
            int(self.last_date[6:]),
            int(self.last_date[3:5]),
            int(self.last_date[0:2]),
        )
...
class ProcessosHonorarios(models.Model):
    ...
    advogado_responsavel = models.ForeignKey(
        ...
        null=True,
        blank=False,
        related_name='lawsuit_values_lawyer',
    )
