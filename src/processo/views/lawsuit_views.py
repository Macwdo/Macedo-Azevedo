...
from processo.forms import LawsuitFileForm, LawsuitForm, LawsuitValuesForm
...
def lawsuit_create(request: HttpRequest):
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'O Processo de código {lawsuit.codigo_processo} foi criado com sucesso.',
            )
            return redirect(
                reverse(
                    'lawsuit:lawsuit_detail', kwargs={'lawsuit_id': lawsuit.pk}
                )
            )
...
def lawsuit_list(request: HttpRequest):
    ...
    context = {'lawsuits': lawsuits, 'page': page}
...
def lawsuit_detail(request: HttpRequest, lawsuit_id: int):
    ...
    if q_files:
        lawsuit_files = lawsuit_files.filter(
            Q(nome_do_anexo__icontains=q_files) | Q(arquivo__icontains=q_files)
        )
...
def lawsuit_edit(request: HttpRequest, lawsuit_id: int):
    ...
    if request.method == 'GET':
        ...
        context = {'form': form, 'lawsuit': lawsuit}
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'O Processo de código {lawsuit.codigo_processo} foi editado com sucesso.',
            )
            return redirect(
                reverse(
                    'lawsuit:lawsuit_detail', kwargs={'lawsuit_id': lawsuit.pk}
...
def lawsuit_delete(request: HttpRequest, lawsuit_id: int):
    ...
    try:
        ...
        messages.error(
            request, f'Não foi possível deletar o processo {lawsuit}'
        )
...
def lawsuit_revert_state(request, lawsuit_id: int):
    ...
    try:
        ...
        messages.success(
            request,
            f'Revertida com sucesso a situação do processo de numero {lawsuit.codigo_processo}!',
        )
    except:
        messages.error(
            request,
            f'Erro ao finalizar processo de numero {lawsuit.codigo_processo}',
        )
    return redirect(
...
def lawsuit_finalize(request, lawsuit_id: int):
    ...
    try:
        ...
        messages.success(
            request,
            f'Proceso de numero {lawsuit.codigo_processo} finalizado com sucesso!',
        )
    except:
        messages.error(
            request,
